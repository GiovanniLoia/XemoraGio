...
-Option 1: Force can_base label -> This might rarely make sense, since you can improperly label data? 
-Option 2: Use whatever data is present -> This might be most accurate? 
...
if regenerate_basecall == True: 
    mod_aligned_path = preprocessing_reads(mod_raw_list[0], xna_ref_fasta, 'modified', xna_raw_dir, mod_pod_dir, mod_fastq_dir, mod_bam_dir, ref_dir)
    can_aligned_path = preprocessing_reads(can_raw_list[0], dna_ref_fasta, 'canonical', dna_raw_dir, can_pod_dir, can_fastq_dir, can_bam_dir, ref_dir)
else: 
...
can_read_xna_pos_all = find_xna_positions(can_cigar_tuples, df_can['Sequence'].tolist(), df_can['Position'].tolist(), can_xna_pos, can_xna_label, df_can['Query Name'].tolist(), df_can['Flag'].tolist())
