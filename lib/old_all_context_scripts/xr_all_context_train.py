...
working_dir = '/home/marchandlab/github/jay/xemora_randomer_troubleshooting/double_alignment_development/240502_BSn_all_strands_larger_dataset'
        ...
        for op, length in cigar_tuples:
            if op == 'S':
                ...
                if ref_base <= xna_pos < ref_base + length: #original if ref_base <= xna_pos_1_based < ref_base + length:
                    if strand == '-':
                        results.append((len(aligned_seq)-current_base- 1 if current_base > 0 else 0, readID, strand, aligned_seq[current_base]))
                    else:
                        results.append((current_base - 1 if current_base > 0 else 0, readID, strand, aligned_seq[current_base-1])) #allow deletions 
...
def filter_by_base(xna_positions, specific_base):
    ...
    for xna_pos, readID, strand, base in xna_positions:
        if base == specific_base:
            ...
            if specific_base in purines or specific_base in pyrimidines
...
def filter_by_strand(xna_positions, desired_strand):
    ...
    for xna_pos, readID, strand, base in xna_positions:
        ...
        print('desired_strand', desired_strand)
        print(strand)
        #if desired_strand == strand: 
        filtered_results.append((xna_pos,readID, strand, base))
    return filtered_results
mod_read_xna_pos = filter_by_strand(mod_read_xna_pos_all, desired_strand)
can_read_xna_pos = filter_by_strand(can_read_xna_pos_all, desired_strand)
...
mod_filter_sam, mod_reads_fasta = sam_filter_fasta_gen(mod_aligned_path, ref_dir, mod_full_bed_path, 'mod')
can_filter_sam, can_reads_fasta = sam_filter_fasta_gen(can_aligned_path, ref_dir, can_full_bed_path, 'can')
...
if regenerate_chunks == True:
    ...
    cmd = 'remora \
      ...
      '+os.path.join(mod_pod_dir,'merged.pod5')+' \
       '+mod_final_bam + ' \
      --output-remora-training-file '+os.path.join(chunk_dir,'mod_chunks.npz')+' \
      --focus-reference-positions '+mod_full_bed_path+' \
      --mod-base '+mod_base+' '+mod_base+' \
      --kmer-context-bases '+kmer_context+' \
      --refine-kmer-level-table '+kmer_table_path+' \
      --refine-rough-rescale '+' \
...
if regenerate_chunks == True: 
    ...
    cmd = 'remora \
      ...
      '+os.path.join(can_pod_dir,'merged.pod5')+' \
       '+can_final_bam + ' \
      --output-remora-training-file '+os.path.join(chunk_dir,'can_chunks.npz')+' \
      --focus-reference-positions '+can_full_bed_path+' \
      --mod-base-control \
      --kmer-context-bases '+kmer_context+' \
      --refine-kmer-level-table '+kmer_table_path+' \
      --refine-rough-rescale '+' \
